...
interface data{
  id: number, name: string, size?: string, last_modified?: string;
}
@Component({
...
export class FileManagerComponent implements OnInit {
  public lstFiles!: Array<data>;
  public searchFilterList!: Array<data>;
  public lstFileList!: Array<data>;
  public lstSearchList!: Array<data>;
  ...
  ngOnInit() {
    ...
    this.lstFileList = [
      ...
      {
        id: 5, name: 'Update.pdf', size: '32.25kb', last_modified: '14-Aug-2023'
      },
      {
      ...
      {
        id: 10, name: 'index.html', size: '49.13kb', last_modified: '24-Sep-2023'
      },
      {
  ...
  searchBName(searchValue: string) {
    this.searchFilterList = this.lstFiles.filter((res: { name: string; }) => {
  ...
  searchText(searchValue: string) {
    this.lstSearchList = this.lstFileList.filter((res: { name: string; }) => {
      return res.name.toLowerCase().includes(searchValue.toLowerCase());
    });
  }
}
